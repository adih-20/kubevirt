...
func HandleStandaloneMode(domainManager virtwrap.DomainManager, config *virtlauncher.VirtLauncherConfig) {
	if config.StandaloneVMI != "" {
		var vmi v1.VirtualMachineInstance
		// Try YAML unmarshal
		if err := yaml.Unmarshal([]byte(config.StandaloneVMI), &vmi); err != nil {
			// Fallback to JSON if YAML fails
			if jsonErr := json.Unmarshal([]byte(config.StandaloneVMI), &vmi); jsonErr != nil {
