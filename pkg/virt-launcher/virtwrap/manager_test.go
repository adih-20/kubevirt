...
import (
	...
	envconfig "kubevirt.io/kubevirt/pkg/virt-launcher/env-config"
...
var _ = Describe("Manager", func() {
	...
	newLibvirtDomainManagerDefault := func() (DomainManager, error) {
		...
		return NewLibvirtDomainManager(mockLibvirt.VirtConnection, testVirtShareDir, testEphemeralDiskDir, nil, "/usr/share/OVMF", ephemeralDiskCreatorMock, metadataCache, nil, virtconfig.DefaultDiskVerificationMemoryLimitBytes, fakeCpuSetGetter, false, envconfig.ReadVirtLauncherConfig())
	}
	...
	Context("on successful VirtualMachineInstance sync", func() {
		...
		It("should unpause a VirtualMachineInstance", func() {
			...
			manager, _ := NewLibvirtDomainManager(mockLibvirt.VirtConnection, "fake", "fake", nil, "/usr/share/OVMF", ephemeralDiskCreatorMock, metadataCache, nil, virtconfig.DefaultDiskVerificationMemoryLimitBytes, fakeCpuSetGetter, false, envconfig.ReadVirtLauncherConfig())
		...
		It("should not add discard=unmap if a disk is preallocated", func() {
			...
			mockLibvirt.DomainEXPECT().GetXMLDesc(libvirt.DomainXMLFlags(2)).MaxTimes(1).Return(string(domainXMLWithInterfaces), nil)
			manager, _ := newLibvirtDomainManager(mockLibvirt.VirtConnection, "fake", "fake", nil, "/usr/share/OVMF", ephemeralDiskCreatorMock, mockDirectIOChecker, metadataCache, nil, virtconfig.DefaultDiskVerificationMemoryLimitBytes, fakeCpuSetGetter, false, envconfig.ReadVirtLauncherConfig())
		...
		It("should hotplug a disk if a volume was hotplugged", func() {
			...
			mockLibvirt.DomainEXPECT().GetXMLDesc(libvirt.DomainXMLFlags(2)).MaxTimes(1).Return(string(domainXMLWithInterfaces), nil)
			manager, _ := newLibvirtDomainManager(mockLibvirt.VirtConnection, testVirtShareDir, testEphemeralDiskDir, nil, "/usr/share/OVMF", ephemeralDiskCreatorMock, mockDirectIOChecker, metadataCache, nil, virtconfig.DefaultDiskVerificationMemoryLimitBytes, fakeCpuSetGetter, false, envconfig.ReadVirtLauncherConfig())
			newspec, err := manager.SyncVMI(vmi, true, &cmdv1.VirtualMachineOptions{VirtualMachineSMBios: &cmdv1.SMBios{}})
			Expect(err).ToNot(HaveOccurred())
			Expect(newspec).ToNot(BeNil())
		})
		...
		It("should return a VirtualMachineInstance launch measurement", func() {
			...
			manager, _ := NewLibvirtDomainManager(mockLibvirt.VirtConnection, testVirtShareDir, testEphemeralDiskDir, nil, ovmfDir, ephemeralDiskCreatorMock, metadataCache, nil, virtconfig.DefaultDiskVerificationMemoryLimitBytes, fakeCpuSetGetter, false, envconfig.ReadVirtLauncherConfig())
	...
	Context("on successful VirtualMachineInstance kill", func() {
		DescribeTable("should try to undefine a VirtualMachineInstance in state",
			func(state libvirt.DomainState) {
				...
				manager, _ := NewLibvirtDomainManager(mockLibvirt.VirtConnection, "fake", "fake", nil, "/usr/share/", ephemeralDiskCreatorMock, metadataCache, nil, virtconfig.DefaultDiskVerificationMemoryLimitBytes, fakeCpuSetGetter, false, envconfig.ReadVirtLauncherConfig())
	...
	Context("on failed GetDomainSpecWithRuntimeInfo", func() {
		...
		Context("on call to GetGuestOSInfo", func() {
			...
			BeforeEach(func() {
				...
				libvirtmanager, _ = NewLibvirtDomainManager(mockLibvirt.VirtConnection, testVirtShareDir, testEphemeralDiskDir, &agentStore, "/usr/share/OVMF", ephemeralDiskCreatorMock, metadataCache, nil, virtconfig.DefaultDiskVerificationMemoryLimitBytes, fakeCpuSetGetter, false, envconfig.ReadVirtLauncherConfig())
			})
	...
	It("executes GetGuestInfo", func() {
		...
		manager, _ := NewLibvirtDomainManager(mockLibvirt.VirtConnection, testVirtShareDir, testEphemeralDiskDir, &agentStore, "/usr/share/OVMF", ephemeralDiskCreatorMock, metadataCache, nil, virtconfig.DefaultDiskVerificationMemoryLimitBytes, fakeCpuSetGetter, false, envconfig.ReadVirtLauncherConfig())
